...
${SCRIPTNAME}
        ...
        systemd-run --unit nssvc1 ip netns exec test /usr/bin/v2ray -c /etc/v2ray/config.json
        # # nsenter/ip netns different
        nsenter --net=/var/run/netns/nsname # # !!! /etc/netns/<name>/resolv.conf, not effect
