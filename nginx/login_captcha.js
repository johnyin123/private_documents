...
function captcha_check(res, real_login) {
  ...
  xhr.onreadystatechange = function() {
    ...
    msg = new Dialog();
    msg.alert('Captcha server error:!' + xhr.status, {}).then((res) => {  console.log(res); });
    return;
  }
  xhr.send(sendObject);
}
...
function TextPopup(payload, real_login, data) {
  dialog.open({
     ...
     template:'<img src="data:' + data.mimetype + ';base64,' + data.img + '"/>' +
        '<label>Captcha<input type="text" name="ctext" value=""></label>' +
...
function getCaptcha(payload, real_login, url) {
  console.log('captcha :' + url + typeof(real_login));
  xhr=new XMLHttpRequest();
...
function captcha_login(real_login, payload) {
  if (payload == "") {
      msg = new Dialog();
      msg.alert('login payload NULL!', {}).then((res) => {  console.log(res); });
      return;
  }
