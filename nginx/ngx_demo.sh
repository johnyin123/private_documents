...
server {
    ...
    http2 on;  # HTTP/2
    listen 443 quic default_server reuseport;    # UDP listener for QUIC+HTTP/3
...
server {
    ...
    location @autoindex {
        xslt_stylesheet /etc/nginx/http-available/index_template.xslt path='$uri';
    }
}
EOF
...
# # change inactive timeout: proxy_cache_valid 200  5m;
...
EOF
cat <<'EOF' > jwt_mod.conf
js_include /etc/nginx/jwt_mod.js;
server {
    listen 443 ssl;
    server_name _;
    location /private {
