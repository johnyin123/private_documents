...
VERSION+=("11a94e95[2026-05-18T12:07:50+08:00]:mk_nginx.sh")
...
CC_OPTS=${CC_OPTS:-"-O2 -fstack-protector-strong -Wformat -Werror=format-security -fPIC -I${MYLIB_DEPS}/include -I${MYLIB_DEPS}/include/libxml2"}
LD_OPTS=${LD_OPTS:-"-Wl,-Bstatic -lsqlite3 -lcrypt -lbrotlienc -lbrotlidec -lbrotlicommon -Wl,-Bdynamic -Wl,-z,relro -Wl,-z,now -fPIC -L${MYLIB_DEPS}/lib -lxml2 -lm"}
...
write_file "${OUTDIR}/etc/nginx/http-conf.d/proxy_pass_header.conf" <<'EOF'
...
log_format json escape=json '{"node":"$hostname","scheme":"$scheme","http_host":"$http_host","server_port":$server_port,"upstream_addr":"$upstream_addr",'
    ...
    '"remote_addr":"$remote_addr","remote_user":"$remote_user","time_iso8601":"$time_iso8601",'
    '"request_method":"$request_method","uri":"$uri","args":"$args","server_protocol":"$server_protocol",'
...
# # Send logs to Logstash, can both local file and syslog, syslog:server=unix:/dev/log
