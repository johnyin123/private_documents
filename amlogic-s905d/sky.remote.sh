cat <<'EOF' > ${OUTDIR}/etc/remote/osd.sh
#!/usr/bin/env bash
color=${1}
shift
...
cat <<'EOF' > ${OUTDIR}/etc/remote/remoter.sh
#!/usr/bin/env bash
...
#############default start#########################################
change_mode() {
    /usr/sbin/th-cmd --socket /var/run/thd.socket --mode ${1}
    /usr/bin/systemd-run --unit osd --uid=${USER} -E TM=100 -E DISPLAY=:0 /etc/remote/osd.sh red "MODE: ${1}"
}
default_main() {
    local key=${1}
    local POWER_CNT=0
...
smplayer_start_stop() {
    local action=start
    pgrep -u ${USER} smplayer>/dev/null && action=stop
    log "smplayer action ${action}"
    case "${action}" in
        start) [ -e "${LIST_FILE}" ] || systemd-run --scope --uid=${USER} /usr/bin/touch ${LIST_FILE}
...
#############media end#########################################
# TH_VALUE=1 TH_EVENT=KEY_POWER /usr/bin/remoter.sh default/media
main() {
    local mode=${1}
    case "${mode}" in
...
KEY_POWER@             1   /usr/bin/systemctl poweroff
