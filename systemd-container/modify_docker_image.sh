...
for arch in ${ARCH[@]}; do
    ...
    for cnt in $(seq 1 5); do
        docker cp --archive badboy-${arch}:${IMG_FILE} - > $(date +'%Y%m%d%H%M%S').bak.tar 2>/dev/null || { echo "wait $cnt"; sleep 2; continue; }
        break
    done
    # # for keep perm and directory position
    (cd "${LOCAL_DIR}"  && tar c .) | docker cp - badboy-${arch}:${IMG_FILE} && echo "COPY OK" || { echo "ERROR"; exit 1; }
