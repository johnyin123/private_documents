...
debian_grub_init() {
    ...
    cat /etc/default/grub > /etc/default/grub.orig
...
vm.dirty_ratio = 60
EOF
    [ -f "/etc/sysctl.d/99-sysctl.conf" ] || ln -s /etc/sysctl.conf /etc/sysctl.d/99-sysctl.conf
}
...
debian_zswap_init() {
    local size_mb=$1
    echo 'can choose systemd-zram-generator'
...
debian_vim_init() {
    ...
    echo "mkdir -p ~/.vim/pack/plugins/start && cd ~/.vim/pack/plugins/start && git clone --depth 1 https://github.com/dhruvasagar/vim-table-mode.git"
...
func SetTitle()
    ...
    if expand ("%:e") == 'py'
        ...
        call setline(6, "logging.basicConfig(encoding='utf-8', format='[%(funcName)s@%(filename)s(%(lineno)d)]%(name)s %(levelname)s: %(message)s', level=os.getenv('LOG', 'WARN').upper())")
        call setline(7, "logger = logging.getLogger(__name__)")
        call setline(8, "# logger.setLevel('INFO')")
        call setline(9, "")
        call setline(10, "class MyClass(object):")
        call setline(11, "    def __init__(self):")
        call setline(12, "        logger.debug('DEMO')")
        call setline(13, "")
        call setline(14, "    def create(self, text:str, val:int=10) -> Optional[Dict]:")
        call setline(15, "        return None")
        call setline(16, "")
        call setline(17, "def main():")
        call setline(18, "    return 0")
        call setline(19, "")
        call setline(20, "if __name__ == '__main__':")
        call setline(21, "    exit(main())")
    endif
    if expand ("%:e") == 'h'
        ...
        call setline(6, "")
        call setline(7, "#ifdef HAVE_CONFIG_H
        call setline(8, "#  include "config.h"
        call setline(9, "#endif
        call setline(10, "")
        call setline(12, "#include <stdio.h>")
        call setline(12, "#define debugln(fmt,args...) fprintf(stderr, \"%s:%d == \"fmt\"\\n\", __FILE__, __LINE__, ##args) /* #define debugln(...) fprintf(stderr, __VA_ARGS__) #define debugln(M,...) fprintf(stderr, \"%s (func %s):%d: \" M \"\\n\", __FILE__, __FUNCTION__, __LINE__, ##__VA_ARGS__) */")
        call setline(13, "")
        call setline(14, "#ifdef __cplusplus")
        call setline(15, "}")
    ...
    if expand ("%:e") == 'c'
        ...
        call setline(10, "};")
        call setline(11, "#define debugln(...) if(g_env.verbose) fprintf(stderr, __VA_ARGS__)")
        ...
        call setline(31, "    int opt, option_index;")
        call setline(32, "    while ((opt = getopt_long(argc, argv, opt_short, opt_long, &option_index)) != -1) {")
        call setline(33, "        switch (opt) {")
        ...
        call setline(43, "        }")
        call setline(44, "    }")
        call setline(45, "    return 0;")
        call setline(46, "}")
        call setline(47, "int main(int argc, char *argv[]) {")
        call setline(48, "    parse_command_line(argc, argv);")
        call setline(49, "    return 0;")
        call setline(50, "}")
    endif
endfunc
EOF
...
log_begin_msg "mount overlay: ${MNT_OPTS:+,${MNT_OPTS}}"
log_success_msg "mount -t overlay overlay -olowerdir=/overlay/lower,upperdir=/overlay/upper,workdir=/overlay/work${MNT_OPTS:+,${MNT_OPTS}} ${rootmnt}"
log_end_msg
