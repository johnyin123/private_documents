...
return 1; }
    ...
    /* 3. Extract the Program File Descriptor directly from the skeleton fields*/
    int prog_fd = bpf_program__fd(skel->progs.trace_dns);
    if (prog_fd < 0) {
        log_error("BPF program FD lookup failed");
        goto cleanup;
    }
    /* 4. Create and attach to a standard network raw socket*/
    ...
    if (setsockopt(sock_fd, SOL_SOCKET, SO_ATTACH_BPF, &prog_fd, sizeof(prog_fd)) < 0) {
        perror("setsockopt SO_ATTACH_BPF failed");
        goto cleanup;
    }
