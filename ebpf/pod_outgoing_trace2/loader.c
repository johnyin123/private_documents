...
static int handle_conn_event(void *ctx, void *data, size_t data_sz) {
    ...
    switch (e->protocol) {
        case IPPROTO_UDP:
            ...
            break;
        case IPPROTO_TCP:
            fprintf(stderr, "T:%s(%d:%s), %s:%d=>%s:%d, cgid=%llu:%llu, pid=%d, len=%d\n",
                    e->comm, e->err, strerror(e->err), src, e->sport, dst, e->dport, e->cgroup_id, e->netns_cookie, e->pid, e->payload_len);
            break;
    }
