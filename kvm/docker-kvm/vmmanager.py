...
class VMManager:
    ...
    def ipaddr(method:str, host:utils.AttrDict, uuid:str)->str:
        def convert_data(data):
            return {value['hwaddr'] if value['hwaddr'] else 'n/a': {'name': name, 'addrs': [{'addr':addr['addr'],'type':{0:'ipv4',1:'ipv6'}.get(addr['type'],'?')}for addr in value['addrs']]} for name, value in data.items() if name != 'lo' and value['addrs'] is not None}
        with libvirt_connect(host.get('url')) as conn:
            dom = conn.lookupByUUIDString(uuid)
