...
{%- macro getdev() %}{%- if vm_disk_bus == 'ide' %}hda{%- else %}sda{%- endif %}{%- endmacro %}
<domain type='kvm'>
...
{%- if vm_uefi is defined and vm_uefi != '' %}
    <loader readonly='yes' secure='{{ vm_uefi_secure | default("no", true) }}' type='pflash'>{{ vm_uefi }}</loader>
{%- endif %}
    <boot dev='hd'/>
    <boot dev='cdrom'/>
    <boot dev='network'/>
...
{%- endif %}
{%- if vm_arch == 'x86_64' %}
    <video><model type='{{ vm_video | default("vga", true)}}' vram='16384' heads='1' primary='yes'/></video>
    <sound model='{{ vm_sound | default("ac97", true) }}'/>
