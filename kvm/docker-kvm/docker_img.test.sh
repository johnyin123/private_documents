...
tag=simplekvm:v1-amd64
username=simplekvm
token_dir=/dev/shm/simplekvm/token
out_dir=/dev/shm/simplekvm/work
etcd_prefix=/simple-kvm/work
ROOTFS=rootfs
VENV=/home/simplekvm/venv/bin/
cat <<EOF
# PKG+="iproute2,busybox" 
# debootstrap --verbose --no-check-gpg --variant=minbase --include=\${PKG} stable ${ROOTFS} https://mirrors.aliyun.com/debian

apt -y --no-install-recommends install jq openssh-client socat qemu-utils qemu-block-extra supervisor python3 python3-venv
apt -y --no-install-recommends install libbrotli1 libgeoip1 libxml2 libxslt1.1 libjansson4 libsqlite3-0 libldap2 libjwt2
...
apt -y --no-install-recommends install python3-libvirt python3-protobuf python3-markupsafe python3-certifi python3-charset-normalizer python3-requests python3-urllib3
cat <<EO_PIP | grep -v '^\s*#.*$' > requires.txt
gunicorn
Flask
pycdlib
websockify
etcd3
# etcd3 use grpcio someversion bug when Docker
# grpcio==1.74.0
EO_PIP
pip install ${PROYX:+--proxy ${PROXY} }-r requires.txt
EOF
sed "s/^user .*;/user ${username} ${username};/g'" ${ROOTFS}/etc/nginx/nginx.conf
cat <<'EODOC' | grep -v '^\s*#.*$' > ${ROOTFS}/etc/nginx/http-enabled/simplekvm.conf
...
EODOC
cat <<EODOC > ${ROOTFS}/etc/supervisord.conf
[supervisord]
nodaemon=true
user=root
...
for i in /var/lib/apt/* /var/cache/*; do
    rm -rf ${ROOTFS}/${i}
done
find ${ROOTFS}/usr/share/locale -maxdepth 1 -mindepth 1 -type d ! -iname 'zh_CN*' ! -iname 'en*' | xargs -I@ rm -rf @ || true

(cd ${ROOTFS} && tar c . | xz > ../rootfs.tar.xz)
cat <<'EOF' > Dockerfile
FROM --platform=amd64 scratch
ADD rootfs.tar.xz /
LABEL maintainer="johnyin" name="simplekvm-amd64" build-date="20250904084215"
...
EOF
docker build --no-cache --force-rm -t ${tag} .
tgz=simplekvm.tgz
rm -f ${tgz} && docker save ${tag} | gzip > ${tgz}
