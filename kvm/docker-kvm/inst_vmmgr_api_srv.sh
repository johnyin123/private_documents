...
VERSION+=("aa1e43f1[2025-05-27T14:04:25+08:00]:inst_vmmgr_api_srv.sh")
################################################################################
FILTER_CMD="cat"
LOGFILE=
...
gunicorn --env LOG=ERROR -b 127.0.0.1:5009 --preload --workers=2 --threads=2 --access-logformat '%(r)s %(s)s msec=%(M)s len=%(B)s' --access-logfile='-' 'main:app'
EODOC
}
inst_app_outdir() {
    local outdir="${1}"
