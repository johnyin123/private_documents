...
VERSION+=("c4af9f9d[2025-05-23T09:11:46+08:00]:inst_vmmgr_api_srv.sh")
################################################################################
FILTER_CMD="cat"
LOGFILE=
...
APPDBS=(devices.json golds.json hosts.json iso.json guests.json ippool.json vars.json flavors.json)
...
gunicorn --env LOG=ERROR -b 127.0.0.1:5009 --preload --workers=2 --threads=2 --access-logformat '%(m)s %(s)s %(U)s msec=%(M)s len=%(B)s' --access-logfile='-' 'main:app'
EODOC
}
inst_app_outdir() {
    local outdir="${1}"
...
gen_app_database() {
    ...
    [ "${dbmode}" == "db" ] && {
        ...
        install -v -C -m 0644 --group=${gid} --owner=${uid} flavors.json ${outdir}/flavors.json
    }
    return 0
}
post_check() {
